...
public class APIPatientTest {
    ...
    @Test
    @WithMockUser ( username = "antti", roles = { "PATIENT" } )
    public void testPatientPreference () throws Exception {
        final PatientForm patient = new PatientForm();
        patient.setAddress1( "1 Test Street" );
        patient.setAddress2( "Some Location" );
        patient.setBloodType( BloodType.APos.toString() );
        ...
        final long pharmId = 12;
        final PatientPreferenceForm prefs = new PatientPreferenceForm( true, pharmId );

        final Patient antti = new Patient( patient );
        antti.setPreferences( prefs );
        antti.save(); // create the patient if they don't exist already

        // tests the request to get a patients preferences
        mvc.perform( get( "/api/v1/patient/preference" ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) );

        final PatientPreferenceForm prefs2 = new PatientPreferenceForm( false, pharmId );

        // tests the request to update a patient's preferences
        mvc.perform( put( "/api/v1/patient/preference" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( prefs2 ) ) ).andExpect( status().isOk() );

        // tests the request to delete a patient's preferences
        mvc.perform( delete( "/api/v1/patient/preference" ) ).andExpect( status().isOk() );

        // tests the request to get a patients preferences after the preferences
        // are deleted
        mvc.perform( get( "/api/v1/patient/preference" ) ).andExpect( status().isNotFound() );
