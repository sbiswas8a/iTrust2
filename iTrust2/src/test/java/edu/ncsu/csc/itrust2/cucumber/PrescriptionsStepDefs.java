...
public class PrescriptionsStepDefs extends CucumberTest {
    ...
    @Given ( "The patient has set preferences and the pharmacy has an assigned pharmacist" )
    public void setDefault () {
        ...
        final Pharmacy p = new Pharmacy( "pharmacy1", "111 Somewhere rd", "00112", State.CA.toString() );
        p.save();
        ...
        pha.selectByVisibleText( "pharmacy1" );
        driver.findElement( By.id( "brand" ) ).click();
        driver.findElement( By.id( "submit" ) ).click();

        // assign pharmacist to the pharmacy
        attemptLogout();
        driver.get( baseUrl );
        username.clear();
        ...
        final Select phar = new Select( driver.findElement( By.id( "selectedPharmacy" ) ) );
        phar.selectByVisibleText( "pharmacy, CA - 00112" );
        final Select pharmacist = new Select( driver.findElement( By.id( "selectedPharmacist" ) ) );
        pharmacist.selectByVisibleText( "pharmacist" );
        driver.findElement( By.id( "submit" ) ).click();

    }

    @Given ( "I log in as the pharmacist" )
    public void pharmacistLogin () {
        attemptLogout();
        driver.get( baseUrl );

        enterValue( "username", "pharmacist" );
    ...
    @When ( "I navigate to the View Prescription page" )
    public void viewPrescription () {
        ( (JavascriptExecutor) driver )
                .executeScript( "document.getElementById('pharmacistViewOfPrescriptions').click();" );
    }

    @When ( "I choose Fill prescription button" )
    public void fillPre () {
        final WebElement fill = driver.findElement( By.id( "fillPrescription" ) );
        fill.click();
    }
    ...
    @When ( "I start documenting an office visit for a patient" )
    public void startVisit () {

        driver.get( VISIT_URL );

        waitForAngular();
        selectItem( "name", this.username );
    }
    ...
    @When ( "I select the pharmacy that match with default" )
    public void selectPharmacy () {
        final WebElement state = driver.findElement( By.id( "pharmacyState" ) );
        final Select dropdown = new Select( state );
        dropdown.selectByVisibleText( "California" );
        final WebElement pharmacy = driver.findElement( By.id( "selectedPharmacy" ) );
        final Select dropdown1 = new Select( pharmacy );
        dropdown1.selectByVisibleText( "pharmacy1" );
    }
