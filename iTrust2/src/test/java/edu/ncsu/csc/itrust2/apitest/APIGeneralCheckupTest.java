...
public class APIGeneralCheckupTest {
    ...
    public void testGeneralCheckupAPI () throws Exception {
        ...
        // Check visits of hcp and patient
        mvc.perform( post( "/api/v1/generalcheckups/hcp/view/" + id ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isOk() );
        mvc.perform( post( "/api/v1/generalcheckups/patient/view/" + id ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isOk() );
