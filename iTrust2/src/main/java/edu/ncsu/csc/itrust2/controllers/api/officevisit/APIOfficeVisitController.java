...
public class APIOfficeVisitController extends APIController {
    ...
    /**
     * Helper method for breaking infinite recursion before serializing
     *
     * @param ov
     *            is the office visit being serialized
     */
    private static void breakRecursion ( final OfficeVisit ov ) {
        if ( !( ov instanceof GeneralCheckup ) ) {
            return;
        }
        final GeneralCheckup gc = (GeneralCheckup) ov;
        // breaking infinite recursion
        if ( gc.getPharmacy() != null ) {
            gc.getPharmacy().nullPrescriptions();
        }
        for ( final Prescription p : gc.getPrescriptions() ) {
            p.setPharmacy( null );
        }
        // breaking infinite recursion
    }
