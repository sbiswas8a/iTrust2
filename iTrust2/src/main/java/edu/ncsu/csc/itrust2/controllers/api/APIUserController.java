...
public class APIUserController extends APIController {
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        try {
            ...
            //// CODE BLOCK FOR CREATING EMPTY PERSONNEL PROFILE FOR PHARMACIST
            //// ////
            if ( user.getRole().equals( Role.ROLE_PHARMACIST ) ) {
                try {
                    final Personnel personnel = Personnel.getEmptyProfile( user );
                    personnel.save();
                    LoggerUtil.log( TransactionType.CREATE_EMPTY_DEMOGRAPHICS, LoggerUtil.currentUser(),
                            user.getUsername(), "Created empty personnel profile for Pharmacist" );
                }
            ...
            //// CODE BLOCK FOR CREATING EMPTY PATIENT PROFILE
            //// ////
            if ( user.getRole().equals( Role.ROLE_PATIENT ) ) {
                try {
                    final Patient patient = Patient.getEmptyProfile( user );
                    patient.save();
                    LoggerUtil.log( TransactionType.CREATE_EMPTY_DEMOGRAPHICS, LoggerUtil.currentUser(),
                            user.getUsername(), "Created empty patient profile" );
                }
                catch ( final Exception e ) {
                    user.delete();
                    return new ResponseEntity(
