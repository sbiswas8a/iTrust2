...
public class APILabProcedureController extends APIController {
     ...
     *
     * @param lp
     *            is the LabProcedure that will be serialized
     */
    private static void breakRecursion ( final LabProcedure lp ) {
        lp.getVisit().setPharmacy( null );
        lp.getVisit().setPrescriptions( null );
    }
    ...
    public ResponseEntity getLabProcedures () {
        ...
        for ( final LabProcedure lp : procs ) {
            breakRecursion( lp );
        }
        // breaking infinite recursion
