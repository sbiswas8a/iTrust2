...
public class APIDiagnosisController extends APIController {
    ...
    /**
     * Helper method to remove circular reference to prevent stack overflow
     * while serializing an object
     *
     * @param diagnosis
     *            is the diagnosis that will be serialized
     */
    private static void breakRecursion ( final Diagnosis diagnosis ) {
        diagnosis.getVisit().setPharmacy( null );
        diagnosis.getVisit().setPrescriptions( null );
    }
    ...
    public ResponseEntity getDiagnosis ( @PathVariable ( "id" ) final Long id ) {
        ...
        if ( d == null ) {
            return new ResponseEntity( errorResponse( "No Diagnosis found for id " + id ), HttpStatus.NOT_FOUND );
        }
    ...
    public List<Diagnosis> getDiagnosesForVisit ( @PathVariable ( "id" ) final Long id ) {
        ...
        final List<Diagnosis> toReturn = Diagnosis.getByVisit( id );
        // breaking infinite recursion
        for ( final Diagnosis d : toReturn ) {
            breakRecursion( d );
        }
        // breaking infinite recursion
        return toReturn;
    ...
    public List<Diagnosis> getDiagnosis () {
        ...
        final List<Diagnosis> toReturn = Diagnosis.getForPatient( self );
        // breaking infinite recursion
        for ( final Diagnosis d : toReturn ) {
            breakRecursion( d );
        }
