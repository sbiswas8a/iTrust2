...
public class APIGeneralCheckupController extends APIController {
    ...
    public ResponseEntity getGeneralCheckup ( @PathVariable ( "id" ) final Long id ) {
        ...
        if ( null == visit ) {
            ...
            // breaking infinite recursion before serializing
            if ( visit.getPharmacy() != null ) {
                visit.getPharmacy().nullPrescriptions();
            }
     ...
     * Also sends any and all prescriptions for the patient to the Pharmacy
    ...
    public ResponseEntity createGeneralCheckup ( @RequestBody final GeneralCheckupForm visitForm ) {
        try {
            ...
            LoggerUtil.log( TransactionType.GENERAL_CHECKUP_CREATE, LoggerUtil.currentUser(),
                    visit.getPatient().getUsername() );
            final Set<Prescription> toSend = visit.getPrescriptions();
            final Pharmacy receiver = visit.getPharmacy();
            if ( !toSend.isEmpty() ) {
                if ( receiver == null ) {
                    visit.delete();
                    for ( final Prescription p : toSend ) {
                        p.delete();
                    }
                    return new ResponseEntity( errorResponse( "Receiver pharmacy not in the system!" ),
                            HttpStatus.NOT_FOUND );
                }
                receiver.addPrescriptions( toSend );
                receiver.save();
                LoggerUtil.log( TransactionType.PRESCRIPTION_SENT, LoggerUtil.currentUser(),
                        visit.getPatient().getUsername(), toSend.size() + " prescriptions sent" );
            }
    ...
    public ResponseEntity updateGeneralCheckup ( @PathVariable final Long id,
        ...
        try {
            ...
            //// REMOVING OLD PRESCRIPTIONS ////
            final Pharmacy oldPharmacy = dbVisit.getPharmacy();
            if ( oldPharmacy != null ) {
                oldPharmacy.removePrescriptions( dbVisit.getPrescriptions() );
                oldPharmacy.save();
            }
            //// REMOVING OLD PRESCRIPTIONS ////
            ...
            //// SENDING NEW PRESCRIPTIONS ////
            final Pharmacy newPharmacy = visit.getPharmacy();
            final Set<Prescription> toSend = visit.getPrescriptions();
            if ( !toSend.isEmpty() ) {
                if ( newPharmacy == null ) {
                    visit.delete();
                ...
                newPharmacy.addPrescriptions( toSend );
                newPharmacy.save();
                LoggerUtil.log( TransactionType.PRESCRIPTION_SENT, LoggerUtil.currentUser(),
                        visit.getPatient().getUsername(), toSend.size() + " prescriptions sent" );
            }
