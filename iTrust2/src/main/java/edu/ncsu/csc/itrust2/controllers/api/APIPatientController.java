...
import edu.ncsu.csc.itrust2.forms.patient.PatientPreferenceForm;
...
public class APIPatientController extends APIController {
    ...
    /**
     * Returns the patient's preference for default pharmacy and prescription
     * type in a PatientPreferenceForm Since the form only contains the id for
     * the pharmacy, further fetching must be done via the Pharmacy API
     *
     * @return returns the server response
     */
    @GetMapping ( BASE_PATH + "/patient/preference" )
    @PreAuthorize ( "hasRole( 'ROLE_PATIENT')" )
    public ResponseEntity getPatientPreference () {
        final Patient current = Patient.getByName( LoggerUtil.currentUser() );
        if ( current == null ) {
            return new ResponseEntity( "Could not fetch logged in user's Patient profile", HttpStatus.NOT_FOUND );
        }
        try {
            final PatientPreferenceForm toReturn = new PatientPreferenceForm( current.getBrandPreference(),
                    current.getDefaultPharmacyId() );
            LoggerUtil.log( TransactionType.PATIENT_PREFERENCES_FETCH, LoggerUtil.currentUser() );
            return new ResponseEntity( toReturn, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            return new ResponseEntity( "Patient has not set preferences", HttpStatus.NOT_FOUND );
        }

    }

    /**
     * Sets the patient's preference for default pharmacy and the prescription
     * type
     *
     * @param ppf
     *            is the form containing the preferences
     * @return returns the server response
     */
    @PutMapping ( BASE_PATH + "/patient/preference" )
    @PreAuthorize ( "hasRole( 'ROLE_PATIENT')" )
    public ResponseEntity setPatientPreference ( @RequestBody final PatientPreferenceForm ppf ) {
        final Patient current = Patient.getByName( LoggerUtil.currentUser() );
        if ( current == null ) {
            return new ResponseEntity( errorResponse( "Could not fetch logged in user's Patient profile" ),
                    HttpStatus.NOT_FOUND );
        }
        current.setPreferences( ppf );
        current.save();
        LoggerUtil.log( TransactionType.PATIENT_PREFERENCES_SET, LoggerUtil.currentUser() );
        return new ResponseEntity( successResponse( "Preferences successfully set!" ), HttpStatus.OK );
    }

    /**
     * Clears the logged in patient's preference for default pharmacy and
     * prescription type
     *
     * @return Returns the server response
     */
    @DeleteMapping ( BASE_PATH + "/patient/preference" )
    @PreAuthorize ( "hasRole( 'ROLE_PATIENT')" )
    public ResponseEntity clearPatientPreference () {
        final Patient current = Patient.getByName( LoggerUtil.currentUser() );
        if ( current == null ) {
            return new ResponseEntity( errorResponse( "Could not fetch logged in user's Patient profile" ),
                    HttpStatus.NOT_FOUND );
        }
        current.clearPreferences();
        current.save();
        LoggerUtil.log( TransactionType.PATIENT_PREFERENCES_CLEAR, LoggerUtil.currentUser() );
        return new ResponseEntity( successResponse( "Preferences cleared!" ), HttpStatus.OK );
    }
